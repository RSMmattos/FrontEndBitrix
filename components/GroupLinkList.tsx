...
export const GroupLinkList: React.FC<GroupLinkListProps> = ({ hideAddButton }) => {
  ...
  const handleSave = async () => {
    try {
      ...
      // Atualiza lista
      const links = await fetch('http://localhost:3000/api/bgcatividade').then(r => r.json());
      setGroupLinks(links);
    } catch (err: any) {
  ...
  const confirmDeleteAction = async () => {
    ...
    try {
      const response = await fetch(`http://localhost:3000/api/bgcatividade/${confirmDelete.codccusto}`, { method: 'DELETE' });
