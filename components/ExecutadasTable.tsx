...
interface ExecutadasTableProps {
  ano: number;
}

const ExecutadasTable: React.FC<ExecutadasTableProps> = ({ ano }) => {
    ...
    const handleExportExcel = () => {
      ...
      XLSX.utils.book_append_sheet(wb, ws, 'Executadas');
      XLSX.writeFile(wb, `executadas_${ano}.xlsx`);
      setExporting(false);
    };
  ...
  useEffect(() => {
    ...
    setError(null);
    Promise.all([
      axios.get(`${API_BASE_URL}/api/batividadeg/data-conclussao-relatorio-v2?ano=${ano}`),
      fetchBitrixGroups()
    ])
      .then(([res, groups]) => {
        setData(res.data);
  ...
  return (
    <>
      ...
      <DetalhesAtividadesModal
        ...
        mes={modalMes}
        contexto={modalMes ? 'executadas' : undefined}
        apiModal={`/api/batividadeg/modal_concluidas/${modalGrupoId}`}
      />
    </>
  );
};

export default ExecutadasTable;
